...
export function calculate(inputString: string): number {
    ...
    while (true) {
        ...
        if (multiplicationIndex < divisionIndex && multiplicationIndex !== -1) {
            inputArray = [
    ...
    while (true) {
        ...
        if (additionIndex < substractionIndex && additionIndex !== -1) {
            inputArray = [
...
function additionRule(firstNumber: string, secondNumber: string): string {
    const result = (parseInt(firstNumber) + parseInt(secondNumber)).toString()
